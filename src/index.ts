...
async function main(args: string[]) {
  ...
  if (!serializedGnProjectExists) {
    gnProject = await GnProject.fromDirectory(perfettoDir, ['mac_debug', 'mac_release']);
    await fs.writeFile(serializedGnProjectPath, GnProject.serialize(gnProject));
  } else {
    gnProject = GnProject.deserialize(await fs.readFile(serializedGnProjectPath, 'utf8'));
  }

  // Write the gyp file
  const result = GypProject.fromGnProject(gnProject, correctPathsForScriptArgs, '//:libperfetto');
  await fs.writeFile(`${perfettoDir}/gypfiles/perfetto.gyp`, result.toGypFile());
